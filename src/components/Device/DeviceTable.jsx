...
const DeviceTable = ({ devices = [], onEdit, onDelete }) => {
  ...
  const columns = useMemo(() => [
    ...
    { id: "location", label: "Ubicación", sortable: true, minWidth: 200, align: "center" },
    { id: "registeredBy", label: "Registrador", sortable: true, minWidth: 200, align: "center" },
    { id: "status", label: "Estado", sortable: true, minWidth: 100, align: "center" },
    { id: "actions", label: "Acciones", sortable: false, minWidth: 120, align: "center" },
  ], []);

  // Función para formatear nombre completo memoizada
  const getFullName = useCallback((user) => {
    if (!user) return "—";
    const parts = [user.name, user.firstSurname, user.secondSurname].filter(Boolean);
    return parts.join(" ") || "—";
  }, []);
