...
export default function JustificationExportButtons({ justifications }) {
  ...
  const handleExportExcel = async () => {
    try {
      ...
      // Agregar datos
      justifications.forEach((attendance) => {
        worksheet.addRow({
      ...
      worksheet.getCell(`B${lastRow}`).value = justifications.length;
      worksheet.getCell(`B${lastRow}`).font = { bold: true, color: { argb: "FF1976D2" } };

      // Estadísticas por tipo
      const entriesCount = justifications.filter((a) => a.type === "IN").length;
      const exitsCount = justifications.filter((a) => a.type === "OUT").length;
      ...
      const onTimeCount = justifications.filter((a) => a.status === "on_time").length;
      const lateCount = justifications.filter((a) => a.status === "late").length;
  ...
  const handleExportPDF = () => {
    try {
      ...
      doc.text(
        `Total: ${justifications.length} | Entradas: ${entriesCount} | Salidas: ${exitsCount} | A tiempo: ${onTimeCount} | Tardanzas: ${lateCount}`,
        14,
        37
      );

      // Preparar datos de la tabla
      const tableData = justifications.map((attendance) => [
  ...
  const isDisabled = !justifications || justifications.length === 0;
