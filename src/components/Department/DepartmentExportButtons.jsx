...
export default function DepartmentExportButtons({ departments }) {
    ...
    try {
      ...
      departments.forEach((department) => {
        worksheet.addRow({
          ...
          status: department.status === "active" ? "Activo" : "Inactivo",
        });
      });
      ...
      saveAs(blob, `departamentos_${new Date().toISOString().split("T")[0]}.xlsx`);
      handleClose();
    } catch (error) {
  ...
  const handleExportPDF = () => {
    try {
      ...
      doc.text("Listado de Departamentos", 14, 20);
      ...
      doc.save(`departamentos_${new Date().toISOString().split("T")[0]}.pdf`);
      handleClose();
    } catch (error) {
  ...
  // Si no hay departamentos, deshabilitar botones
  const isDisabled = !departments || departments.length === 0;
