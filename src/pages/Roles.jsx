...
export default function Roles() {
  ...
  // Evitar llamadas duplicadas eliminando fetchRoles de las dependencias
  useEffect(() => {
  ...
  const handleSubmit = useCallback(
    async (data) => {
      ...
      try {
        const isEditing = !!dialog.editRole;

        if (isEditing) {
          await updateRole(dialog.editRole._id, data);
        ...
        setDialog({ open: false, editRole: null, error: "" });
        await refreshRoles();
      } catch (err) {
        const errorMessage =
  ...
  }, [deleteState.id, showSuccess, showError, refreshRoles]);
  ...
  const handleOpenDialog = useCallback(() => {
    setDialog({ open: true, editRole: null, error: "" });
  }, []);
    ...
    <Box sx={{ width: "100%" }}>
      ...
      >
        <Box sx={{ position: "relative" }}>
          <LoadingOverlay show={loading && roles.length > 0} />
          {roleTableMemo}
        </Box>

        {roles.length > 0 && <Divider />}
